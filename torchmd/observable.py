...
def msd(positions, box):
    ...
    for step in range(1, positions.shape[0]):
        ...
        displacements = torch.where(
            displacements > 0.5 * box,
            displacements - box,
            torch.where(displacements < -0.5 * box, displacements + box, displacements),
        )
        total_displacements = total_displacements + displacements
        # Calculate squared displacements
        total_squared_displacements = (
            torch.linalg.norm(total_displacements, axis=-1) ** 2
        )
...
class DiffusionCoefficient(torch.nn.Module):
    ...
    def forward(self, msd_data):
        ...
        X = torch.stack([X, torch.ones((n,))], dim=1).to(
            self.device
