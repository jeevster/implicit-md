...
LJPARAMS = {"epsilon": 1.0, "sigma": 1.0}

MLPPARAMS = {"D_in": 1, "H": 128, "num_layers": 3, "act": "relu", "D_out": 1}
...
class LJFamily(torch.nn.Module):
    ...
    def LJ(self, r, sigma, epsilon):
        return (
            4 * epsilon * ((sigma / r) ** self.rep_pow - (sigma / r) ** self.attr_pow)
        )
...
class ModifiedMorse(torch.nn.Module):
    ...
    def forward(self, r):
        ...
        pot = (torch.exp(2 * exponent) - 2 * torch.exp(exponent) - self.A) / (
            1 + self.A
        )
    ...
    def __init__(self, rdf_range, rdf, device, kT=1.0):
        ...
        try:
            ...
            raise NotImplementedError(
                "install torch cubic spline with pip"
...
class SplineOverlap(torch.nn.Module):
    ...
    def __init__(self, K, V0, device, n_splines=600, rmax=15.0, rmin=0.00):
        ...
        targ = (
            torch.Tensor(overlap(np.linspace(rmin, rmax, n_splines), K, V0))
            .reshape(-1, 1)
            .to(device)
        )
...
class pairMLP(torch.nn.Module):
    def __init__(
        self, n_gauss, r_start, r_end, n_layers, n_width, nonlinear, res=False
    ):
        ...
        self.smear = GaussianSmearing(
            start=r_start, stop=r_end, n_gaussians=n_gauss, trainable=True
        )

        self.layers = nn.ModuleList(
            [nn.Linear(n_gauss, n_gauss), nlr, nn.Linear(n_gauss, n_width), nlr]
        )
...
class TpairMLP(torch.nn.Module):
    def __init__(
        ...
        self.energy = pairMLP(
            n_gauss, r_start, r_end, n_layers, n_width, nonlinear, res=res
        )
        self.entropy = pairMLP(
            n_gauss, r_start, r_end, n_layers, n_width, nonlinear, res=res
        )
...
class toy2d(torch.nn.Module):
    ...
    def Q(self, d, r):
        ...
        return (
            d
            * (3 * torch.exp(-2 * alpha * (r - r0)) / 2 - torch.exp(-alpha * (r - r0)))
            / 2
        )
    ...
    def J(self, d, r):
        ...
        return (
            d
            * (torch.exp(-2 * alpha * (r - r0)) - 6 * torch.exp(-alpha * (r - r0)))
            / 4
        )
...
class MLP2d(torch.nn.Module):
    def __init__(
        self, D_in=2, H=128, D_out=1, num_layers=3, act="relu", excluded_vol=True
    ):
...
class MLP(torch.nn.Module):
    def __init__(
        self, D_in=1, H=128, D_out=1, num_layers=3, act="relu", excluded_vol=True
    ):
