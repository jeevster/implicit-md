...
class GNNPotentials(GeneralInteraction):
    ...
    def _reset_topology(self, xyz):
        ...
        """
        self.inputs["nbr_list"], offsets = generate_nbr_list(
            xyz, self.cutoff, self.cell, ex_pairs=self.ex_pairs
        )
...
class TPairPotentials(GeneralInteraction):
    ...
    def __init__(
        ...
        nbr_list, offsets = generate_nbr_list(
            torch.Tensor(system.get_positions()).to(self.nbr_list_device),
            self.cutoff,
    ...
    def _reset_topology(self, xyz):
        ...
        Returns:
            TYPE: Description
        """
        nbr_list, pair_dis, offsets = generate_nbr_list(
            xyz.to(self.nbr_list_device),
            self.cutoff,
            self.cell.to(self.nbr_list_device),
            index_tuple=self.index_tuple,
            ex_pairs=self.ex_pairs,
...
class Electrostatics(torch.nn.Module):
    ...
    def __init__(
        self, charges, cell, device=0, cutoff=2.5, index_tuple=None, ex_pairs=None
    ):
        """Summary
    ...
    def forward(self, x):
        ...
        Returns:
            TYPE: Description
        """
        nbr_list, pair_dis, _ = generate_nbr_list(
            x,
            self.cutoff,
            self.cell,
            index_tuple=self.index_tuple,
...
class AnglePotentials(torch.nn.Module):
    ...
    def forward(self, xyz):
        ...
        bond_vec1 = (
            bond_vec1 + get_offsets(bond_vec1, self.cell, self.device) * self.cell
        )
        bond_vec2 = (
            bond_vec2 + get_offsets(bond_vec2, self.cell, self.device) * self.cell
        )
