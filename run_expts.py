...
molecules_md17 = ['naphthalene', 'salicylic_acid', 'aspirin']# 'ethanol', 'toluene', 'benzene', 'salicylic_acid', 'toluene', 'uracil']

molecules_md22 = ['DHA', 'ac_Ala3_NHMe', 'AT_AT', 'stachyose', 'AT_AT_CG_CG'] #'buckyball_catcher', 'double_walled_nanotube']

config_yml = '/global/homes/s/sanjeevr/implicit-md/configs/md17/base.yml'
...
for mol in molecules_md17:
    command = f"python nve_implicit.py --config-yml {config_yml} \
                 --molecule={mol} --name=md17 --size=1k --rdf_loss_weight=1 --vacf_loss_weight=0 --minibatch_size=40 \
                 --energy_loss_weight=0.05 --force_loss_weight=0.95 --exp_name=TEST_resumestrategy0.25_learn_continuous_onlytrainunstable"
    subprocess.run(command.split(" "))

#MD17 energy/force + dynamics
# for mol in molecules_md17:
#     command = f"python nve_implicit.py --config-yml {config_yml} \
#                  --molecule={mol} --results_dir=results_nnip_continuous_dynamics_new --name=md17 --size=1k \
#                  --integrator=Langevin --rdf_loss_weight=1 --vacf_loss_weight=1 --minibatch_size=20 \
#                  --max_frac_unstable_threshold=0.1 --energy_loss_weight=0.05 --force_loss_weight=0.95 --exp_name=TEST_resumestrategy0.25_learn_continuous"
