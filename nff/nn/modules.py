...
class DihedralNet(torch.nn.Module):
    def __init__(self, Fr, Lh, terms=["OPLS"], trainable=False):
        ...
        self.true_params = None
        self.learned_params = {}
        self.nonlinear = ParameterPredictor(
            2 * self.Fr, Lh, Lh[-1], trainable=trainable
        )
        if "multiharmonic" in self.terms:
            self.dihedralnet_multiharmonic = ParameterPredictor(
                Lh[-1], Lh, 5, trainable=trainable
            )
            self.learned_params["multiharmonic"] = {"dihedralnet": None}
        if "OPLS" in self.terms:
            self.dihedralnet_OPLS = ParameterPredictor(
                Lh[-1], Lh, 4, trainable=trainable
            )
    ...
    def forward(self, r, batch, xyz):
        ...
        pair1 = self.nonlinear(
            torch.cat([r[dihedrals[:, 1]], r[dihedrals[:, 0]]], dim=1)
        )
        pair2 = self.nonlinear(
            torch.cat([r[dihedrals[:, 2]], r[dihedrals[:, 3]]], dim=1)
        )
        ...
        if "multiharmonic" in self.terms:
            ...
            for m in range(5):
                ...
                E = E + A * (cos_phi.pow(m))
            self.learned_params["multiharmonic"][
                "dihedralnet"
            ] = multiharmonic_constants.tolist()
        if "OPLS" in self.terms:
...
class ImproperNet(torch.nn.Module):
    ...
    def forward(self, r, batch, xyz):
        ...
        phi = torch.acos(cos_phi / 1.000001)
        pair1 = self.nonlinear(
            torch.cat([r[impropers[:, 0]], r[impropers[:, 1]]], dim=1)
        )
        pair2 = self.nonlinear(
            torch.cat([r[impropers[:, 0]], r[impropers[:, 2]]], dim=1)
        )
        pair3 = self.nonlinear(
            torch.cat([r[impropers[:, 0]], r[impropers[:, 3]]], dim=1)
        )
    ...
    def __init__(self, multitaskdict):
        ...
        for top in ["bond", "angle", "dihedral", "improper", "pair"]:
            if top + "_terms" in multitaskdict.keys():
                self.terms[top] = multitaskdict.get(
                    top + "_terms", default_terms_dict[top + "_terms"]
                )
        ...
        for key in autopology_keys:
            for top in self.terms.keys():
                if top + "_terms" in multitaskdict:
                    topologynet[key][top] = TopologyNet[top](
                        Fr, Lh, self.terms[top], trainable=trainable
                    )

        # module dictionary of the form {"energy_0": {"bond": BondNet0, "angle": AngletNet0},
        # "energy_1": {"bond": BondNet1, "angle": AngletNet1} }
        self.auto_modules = ModuleDict(
            {
                key: ModuleDict(
                    {top: topologynet[key][top] for top in self.terms.keys()}
                )
                for key in autopology_keys
            }
        )
        ...
        self.offset = ModuleDict(
            {key: ParameterPredictor(Fr, Lh, 1) for key in autopology_keys}
        )
        ...
        self.moduledict = ModuleDict(
            {
                ...
                "message_node_filter": Dense(
                    in_features=n_atom_basis, out_features=n_filters
                ),
...
class BondEnergyModule(nn.Module):
    ...
    def forward(self, xyz, bond_adj, bond_len, bond_par):
        ...
        energy = 0.5 * scatter_add(
            src=ebond, index=bond_adj[:, 0], dim=0, dim_size=xyz.shape[0]
        )
        energy += 0.5 * scatter_add(
            src=ebond, index=bond_adj[:, 1], dim=0, dim_size=xyz.shape[0]
        )
