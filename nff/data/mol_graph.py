...
def adjdistmat(atoms, threshold=DISTANCETHRESHOLDICT_Z, unwrap=True):
    ...
    thresholdmat = np.array(
        [
            [threshold.get(tuple(sorted((i, j))), 2.0) for i in atomicnums]
            for j in atomicnums
        ]
    )
...
def pop_molecules(atomsobject, mol_idx, id_list):
    ...
    return Atoms(
        numbers=np.concatenate(new_z),
        positions=np.concatenate(new_pos),
...
def get_single_molecule(atomsobject, mol_idx, single_mol_id):
    ...
    return Atoms(numbers=z, positions=pos, cell=atomsobject.cell, pbc=True)
