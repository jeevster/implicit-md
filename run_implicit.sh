...
if [ "$1" == "water" ]; then
    command_train="python $script --config-yml $config_yml --model=$2 --lr=$3 -rdf_loss_weight=$4 --vacf_loss_weight=$5 \
                --energy_force_loss_weight=$6 --only_learn_if_unstable_threshold_reached=$7 --results_dir=results_water_localneighborhoods"
else
    command_train="python $script --config-yml $config_yml --molecule=$2 --lr=$3 -rdf_loss_weight=$4 --vacf_loss_weight=$5 \
                --energy_force_loss_weight=$6 --only_learn_if_unstable_threshold_reached=$7 --size=$8 --results_dir=results_md17_correctedlogging"
fi
srun -u $command_train
