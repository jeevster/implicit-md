...
class ImplicitMDSimulator():
    ...
    def om_action(self, vel_traj, radii_traj):
        ...
        a_tp1 = self.force_calc(r, atomic_numbers = atomic_numbers, batch = batch, retain_grad = True).reshape(v_t.shape) / self.masses.unsqueeze(1).unsqueeze(1)
        ...
        return diff/self.noise_f.unsqueeze(1).unsqueeze(1), om_action.sum((-3, -2, -1)).flatten()
    ...
    def solve(self):
        ...
        with torch.enable_grad():
            vtraj = velocities_traj.detach().requires_grad_(True)
            rtraj = radii_traj.detach().requires_grad_(True)
            diff, om_act = self.om_action(vtraj, rtraj)
        ...
        def get_grads(v):
            return compute_grad(inputs = list(self.model.parameters()), output = om_act, grad_outputs = v, create_graph = True, allow_unused = True)
        ...
        #naive_backprop_grads_vec = get_grads_vmaped(I_N)
        naive_backprop_grads = [[self.process_gradient(g) for g in torch.autograd.grad(o, self.model.parameters(), create_graph = True, allow_unused = True)] for o in tqdm(om_act.flatten())]
        ...
        # start = time.time()
        # final_adjoints, adjoint_norms, dl_dtheta, R_reversed = self.get_adjoints(radii_traj, velocities_traj, grad_outputs, force_fn = self.force_calc)
        # end = time.time()
        ...
        #     #multiply by self.dt to account for the reverse-ODE timestepping
        ...
        #     grads = [self.process_gradient(g) * self.dt \
        ...
        return naive_backprop_grads_flattened
    ...
    def get_adjoints(self, pos_traj, vel_traj, grad_outputs, force_fn = None):
        with torch.no_grad():
            ...
            a = grad_outputs[:, -1]
            dl_dtheta = torch.cat([torch.zeros_like(param.flatten()).to(self.device) for param in self.model.parameters()])
            for i in tqdm(range(self.vacf_window)):
                ...
                with torch.enable_grad():
                    ...
                    forces = force_fn(R)
                #compute necessary Vector-Jacobian products
                #_, vjp_a = torch.autograd.functional.vjp(force_fn, R, a)
                vjp_a, = torch.autograd.grad(forces, R, a, create_graph = True)
                vjp_theta = torch.autograd.grad(forces, self.model.parameters(), a, create_graph = True, allow_unused = True)
                vjp_theta_flattened = torch.cat([self.process_gradient(v).flatten() for v in vjp_theta])

                #update dl_dtheta
                dl_dtheta += self.dt*vjp_theta_flattened
    
                #update adjoint state
                term1 = a_dt**2 * vjp_a /M
                term2 = a_dt*self.gamma * a
                #term 1 is much larger than term 2, so the sign of term2 doesn't end up mattering
                #should be positive (?) but Martin's paper writes it negative
                a = a + term1 + term2
        ...
        return adjoints, adjoint_norms, dl_dtheta, R_reversed
...
if __name__ == "__main__":
    ...
    naive_backprop_grads = simulator.solve()
