...
def energy_force_gradient(simulator):
    ...
    print(f"Computing gradients of bottom-up (energy-force) objective on {simulator.train_dataset.__len__()} samples")
    gradients = []
    losses = []
    if simulator.model_type == "nequip":
        with torch.enable_grad():
            for data in tqdm(simulator.train_dataloader):
                ...
                data['pbc'] = simulator.atoms_batch['pbc'][0].unsqueeze(0).repeat(actual_batch_size, 1)
                data['atom_types'] = simulator.atoms_batch['atom_types'][0:simulator.n_atoms].repeat(actual_batch_size, 1)
                ...
                # Run model
                data_unscaled['edge_index'] = radius_graph(data_unscaled['pos'].reshape(-1, 3), r=model_config['model'][r_max_key], batch=data_unscaled['batch'], max_num_neighbors=32)
                data_unscaled['edge_cell_shift'] = torch.zeros((data_unscaled['edge_index'].shape[1], 3)).to(device)
                ...
                grads = torch.autograd.grad(loss, simulator.model.parameters(), allow_unused = True)
                gradients.append(process_gradient(simulator.model.parameters(), grads, simulator.device))
                losses.append(loss.detach())
    else:
        with torch.enable_grad():
            for batch in tqdm(simulator.train_dataloader):
                with torch.cuda.amp.autocast(enabled=simulator.trainer.scaler is not None):
                    ...
                    loss = simulator.trainer.scaler.scale(loss) if simulator.trainer.scaler else loss
                    grads = torch.autograd.grad(loss, simulator.model.parameters(), allow_unused = True)
                    gradients.append(process_gradient(simulator.model.parameters(), grads, simulator.device))
                    losses.append(loss.detach())
...
def energy_force_error(simulator):
    if simulator.model_type == 'nequip':
        data_config = f"configs/{simulator.name}/nequip_data_cfg/{simulator.molecule}.yml"
        # call nequip evaluation script
        os.system(f'nequip-evaluate --train-dir {os.path.dirname(simulator.curr_model_path)} \
        ...
        with open(f'{os.path.dirname(simulator.curr_model_path)}/test_metric.log', 'r') as f:
    ...
    #non-Nequip models use OCP calculator
        ...
        calculator = OCPCalculator(config_yml=simulator.model_config, checkpoint=simulator.curr_model_path, 
...
def save_checkpoint(simulator, best=False, name_=None):
    if simulator.model_type == "nequip":
        ...
        checkpoint_path = os.path.join(simulator.save_dir, name)
        new_state_dict = OrderedDict(("module."+k if "module" not in k else k, v) for k, v in model.state_dict().items())
        torch.save({
...
def create_frame(radii, velocities, cell, bonds, pbc, diameter_viz, n_atoms, dt, name, frame):
    ...
    if pbc:
        ...
        if name == 'lips': #account for non-cubic cell
            ...
            radii = ((radii / torch.diag(cell)) % 1) * torch.diag(cell)  - torch.diag(cell)/2 #wrap coords (last subtraction is for cell alignment in Ovito)
...
def thermo_log(radii, velocities, pe, masses, n_atoms, stability_criterion, bond_length_dev, rdf_mae, pbc):
    ...
    results_dict = {"Temperature": temp.item(),
                    ...
                    "Momentum Magnitude": torch.norm(torch.sum(masses*velocities, axis =-2)).item(),
                    'Max Bond Length Deviation': bond_length_dev(radii.unsqueeze(0))[1].mean().item() \
                                                    if pbc else instability.mean().item()}
    if pbc:
