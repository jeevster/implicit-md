...
# Simulation results are in the "inference" subfolder
...
sbatch run_implicit_simulate.sh $system $molecule $lr 1 0 $ef_weight 'post_nhlong' 'NoseHoover' 1000

# Longer Langevin sims for MD17 (pre and post StABlE training models)
sbatch run_implicit_simulate.sh $system $molecule $lr 1 0 $ef_weight 'pre_langevinlong' 'Langevin' 1000
sbatch run_implicit_simulate.sh $system $molecule $lr 1 0 $ef_weight 'post_langevinlong' 'Langevin' 1000
...
sbatch run_implicit_simulate.sh $system $model $lr 1 0 $ef_weight 'pre_langevinlong' 'Langevin' 1000
sbatch run_implicit_simulate.sh $system $model $lr 1 0 $ef_weight 'post_cycle1_langevinlong' 'Langevin' 1000
