...
for model in "${models[@]}"; do
    for lr in "${lrs[@]}"; do
        for ef_weight in "${ef_weights[@]}"; do
            jid1=$(sbatch --parsable run_implicit.sh $system $model $lr 1 0 $ef_weight True)
            # post inference - for every HP combination - can only start once training finishes
            sbatch --dependency=afterany:$jid1 run_implicit_simulate.sh $system $model $lr 1 0 $ef_weight 'post'
        done
    done
    #non-post inferences - once per model (can start after training starts)
    lrtemp=0.001
    ef_weighttemp=1
    sbatch --dependency=after:$jid1 run_implicit_simulate.sh $system $model $lrtemp 1 0 $ef_weighttemp 'pre'
    sbatch --dependency=after:$jid1 run_implicit_simulate.sh $system $model $lrtemp 1 0 $ef_weighttemp '10k'
