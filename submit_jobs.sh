...
#                 sbatch run_implicit_simulate.sh $system $molecule $lr 1 0 $ef_weight 'post_temp750' $size 750
#                 sbatch run_implicit_simulate.sh $system $molecule $lr 1 0 $ef_weight 'pre_temp750' $size 750
#                 sbatch run_implicit_simulate.sh $system $molecule $lr 1 0 $ef_weight 'post_temp1000' $size 1000
#                 sbatch run_implicit_simulate.sh $system $molecule $lr 1 0 $ef_weight 'pre_temp1000' $size 1000
...
for lr in "${lrs[@]}"; do
    for ef_weight in "${ef_weights[@]}"; do
        ...
        # sbatch run_implicit_simulate.sh $system $model $lr 1 0 $ef_weight 'pre'
        # sbatch run_implicit_simulate.sh $system $model $lr 1 0 $ef_weight 'post_cycle1_epoch10'
        # sbatch run_implicit_simulate.sh $system $model $lr 1 0 $ef_weight 'post_cycle1_epoch23'
	sbatch run_implicit_simulate.sh $system $model $lr 1 0 $ef_weight 'post_cycle1'
    done
done
