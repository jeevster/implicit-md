    ...
    for molecule in "${molecules_md22[@]}"; do
        for lr in "${lrs[@]}"; do
            for ef_weight in "${ef_weights[@]}"; do
                ...
                sbatch run_implicit_simulate.sh $system $molecule $lr 1 0 $ef_weight 'pre_temp350' $size 350
                sbatch run_implicit_simulate.sh $system $molecule $lr 1 0 $ef_weight 'post_temp350' $size 350
...
#         sbatch --dependency=afterany:$jid1 run_implicit_simulate.sh $system $model $lr 1 0 $ef_weight 'pre'
#         sbatch --dependency=afterany:$jid1 run_implicit_simulate.sh $system $model $lr 1 0 $ef_weight 'post_cycle1'
#     done
# done
